...
export class InboundTransactionsComponent implements OnInit, OnDestroy {
    ...
    saveInbound(print = false) {
        this.pageLoading = true;
        if (!this.payload.event.supplier || typeof this.payload.event.supplier.supplier_id !== 'string') {
        ...
        payload.event.supplierId = this.payload.event.supplier.supplier_id;
        delete payload.event.supplier;
    ...
    private containsSelectedSupplierId(e: any) {
        if (e.supplier) {
            e.supplier.filter(supplier => {
                if (supplier.id === this.payload.event.supplier.supplier_id) {
                    return true;
                }
            });
        }
                        ...
                        this.searchData.forEach(data => {
                            if (data.id_type.indexOf(this.soaUniqueName.id_type) >= 0) {
                                ...
                                if (Array.isArray(this.suppliers) && this.suppliers.length == 1) {
                                    if (
                                        !this.payload.event.supplier ||
                                        this.payload.event.supplier.supplier_id !== this.suppliers[0].supplier_id
                                    ) {
        ...
        this.searchRawData.forEach(el => {
            if (value.unique_search === el.unique_search) {
                ...
                if (this.suppliers.length == 1) {
                    ...
                    this.payload.event.supplier = null;
                }
    ...
    updateSupplier(event: any) {
        if (Array.isArray(event)) {
            this.payload.event.supplier = event[0];
        } else {
            this.payload.event.supplier = event;
        }
    }
}
