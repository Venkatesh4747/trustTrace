    ...
    InitSubQuestionResponses() {
        this.questions.forEach((question, index) => {
            ...
            if (question.type === 'SINGLE_SELECT') {
                ...
                const optionsLength = this.questions[index].options.length;
                for (let i = 0; i < optionsLength; i++) {
                    if (
                        answered &&
                        answered.length > 0 &&
                        answered.indexOf(this.questions[index]['options'][i]) !== -1
                    ) {
                        if (!Array.isArray(this.checkbox_response[question.id])) {
                            this.checkbox_response[question.id] = [];
                        }
                        this.checkbox_response[question.id].push(this.questions[index]['options'][i]);
                    }
